...
class RefreshTokenRepository:
    ...
    def create_refresh_token(self, token_hash: str, user_agent: str, user_id: int):
        refresh_token = RefreshToken(
            token_hash=token_hash, user_agent=user_agent, user_id=user_id
        )
    ...
    def update_refresh_token(
        self, refresh_token_id: int, **kwargs
    ) -> Type[RefreshToken] | None:
        refresh_token = (
            self.session.query(RefreshToken).filter_by(id=refresh_token_id).first()
        )
