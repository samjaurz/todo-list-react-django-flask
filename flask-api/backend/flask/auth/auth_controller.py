...
def login(session):
    ...
    tokens = JWTAuth().get_access_token({"user_id": user.id, "email": user.email})
    ...
    retrieve_refresh_db = RefreshTokenRepository(session).get_refresh_token_by_user_id(
        user.id
    )
...
def sign_up(session):
    ...
    return jsonify({"message": "Sign up successful", "user_id": created_user.id}), 201
...
def validate_refresh_token(session):
    ...
    tokens = JWTAuth().get_access_token(
        {"user_id": decode["user_id"], "email": decode["email"]}
    )
    ...
    response = jsonify(
        {"message": "Generate token successful", "access_token": tokens["access_token"]}
    )
    response.status_code = 200
    response.set_cookie(
        "refresh_token",
...
def verification(session):
    ...
    UserRepository(session).update_user(user_id=decode["user_id"], **{"status": True})
...
def resend_email_verification(session):
    ...
    return jsonify({"message": "Email sent successfully", "user_id": user.id}), 200
